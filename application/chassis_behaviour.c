...
static void chassis_yaw_set_angle_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector);
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    // 遥控器设置模式
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL])) // 键鼠挡（23.5.5）
    {
        ...
        if ((chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_CTRL) && chassis_move_mode->last_crtl_key == 0)
        {
            chassis_move_mode->LOB_SHOT_FLAG *= -1;
            chassis_move_mode->set_angle_flag = 0;
        }

        chassis_move_mode->last_crtl_key = chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_CTRL;

        chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW; // CHASSIS_NO_FOLLOW_YAW;
        if (chassis_move_mode->LOB_SHOT_FLAG == -1)
        {

            if (chassis_move_mode->set_angle_flag == 0)
            {
            ...
            if (chassis_move_mode->set_angle_flag == 1)
            {
                chassis_behaviour_mode = CHASSIS_NO_MOVE;
            }
    ...
    else if (chassis_behaviour_mode == CHASSIS_ANGLE_SET)
    {
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_SET_ANGLE;
    }
}
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    else if (chassis_behaviour_mode == CHASSIS_VECTOR_SET_ANGLE)
    {
        chassis_yaw_set_angle_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
}
...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    // judge if swing
    // 判断是否要摇摆
    if (switch_is_mid(chassis_move_rc_to_vector->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL])) // (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY)(键鼠挡)
    {
        if (swing_flag == 0)
        {
            swing_flag = 1;
