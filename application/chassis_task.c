...
double REVERSE_FLAG = 1.0; // 键鼠头反向flag
int last_r_key = 0;        // 上一时刻鼠标右键值
double time_a;             // 随机小陀螺时间
int time_i = 0;            // 总时间
bool spinning_state = 0;   // 小陀螺开闭状态
/**
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    // 底盘开机状态为原始
    chassis_move_init->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW; // DANGER
...
static void chassis_set_mode(chassis_move_t *chassis_move_mode)
{
    ...
    chassis_behaviour_mode_set(chassis_move_mode); // 设置底盘行为模式
}
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    else if (switch_is_up(chassis_move_rc_to_vector->chassis_RC->rc.s[0]))
    { // remote control 遥控器档
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    // 设置角度模式：用于固定底盘于一确定位置
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_SET_ANGLE)
    {
        fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
        ...
        // set control relative angle  set-point / calculate ratation speed
        // 设置控制相对云台角度(chassis_relative_angle_set),计算旋转PID角速度(wz_set) ——
        chassis_move_control->chassis_relative_angle_set = 1.57f / 1.9f;
...
static void spinning_move(chassis_move_t *chassis_move_spinnig, fp32 angle_tran)
{
    ...
    */
    ...
    /*
        匀速小陀螺
    */
    chassis_move_spinnig->wz_set = 10.0f;//(-2.5f * PID_calc(&chassis_move_spinnig->chassis_angle_pid,
