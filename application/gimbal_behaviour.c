...
static void gimbal_lob_shot_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
/**
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
	...
	else if (gimbal_behaviour == GIMBAL_LOB_SHOT)
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
    }
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    {
        gimbal_lob_shot_control(add_yaw, add_pitch, gimbal_control_set);
    }
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    {
		if((gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_CTRL)&&gimbal_mode_set->last_crtl_key==0)
		{
			gimbal_mode_set->LOB_SHOT_FLAG *= -1;			
		}
		
		gimbal_mode_set->last_crtl_key = gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_CTRL;
        ...
        else
        {
			...
			if(gimbal_mode_set->LOB_SHOT_FLAG == -1)
			{
				gimbal_behaviour = GIMBAL_LOB_SHOT;
			}
        }
...
static void gimbal_auto_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    set_autoaim_angle(yaw, pitch, absolute_yaw_set, absolute_pitch_set,gimbal_control_set->autoaim_data->yaw,gimbal_control_set->autoaim_data->pitch);
}
...
static void gimbal_lob_shot_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    static int16_t yaw_channel = 0, pitch_channel = 0;
		...
		if(switch_is_mid(gimbal_control_set->gimbal_rc_ctrl->rc.s[0])){	//keyboard control 键鼠档
			...
			{
				if((gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_UP_KEY))
				{
					*pitch += PITCH_MOUSE_SEN;			
				}
				else if((gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_DOWN_KEY))
				{
					*pitch -= PITCH_MOUSE_SEN;			
				}
				if((gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_LEFT_KEY))
				{
					*yaw += YAW_MOUSE_SEN*5.0f;			
				}
				if((gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_RIGHT_KEY))
				{
					*yaw -= YAW_MOUSE_SEN*5.0f;			
				}
//				*yaw = -gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN;
